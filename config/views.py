...
# Global state management (Simple in-memory storage for dev server)
streaming = False
ffmpeg_process = None
...
def webcam_project(request):
    """Renders the main dashboard HTML."""
    return render(request, "webcam_project.html")

def start_stream(request):
    """
    Starts the FFmpeg subprocess to push webcam video to the RTSP server.
    Auto-detects macOS (AVFoundation) vs Linux (V4L2).
    """
    ...
    # Platform detection for camera input
    ...
    # FFmpeg command to push video to RTSP
    ffmpeg_command = [
        "ffmpeg", *camera_input,
...
def stop_stream(request):
    """Terminates the FFmpeg subprocess."""
    global streaming, ffmpeg_process
    if ffmpeg_process:
        try:
...
def video_feed(request):
    """Route for the <img> tag src to display video."""
